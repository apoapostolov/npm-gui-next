...
import { ChangelogCell } from './table-cells/ChangelogCell';
...
const columnsGlobal = columns.filter(column => ['name', 'homepage', 'repo', 'changelog', 'npm','score','size', 'updated', 'to', 'install', 'installed', 'latest', 'actions'].includes(column.name))
...
export const Dependencies: FC<Props> = ({ projectPath }) => {
  ...
  return (
    <>
      <DependenciesHeader
        displayedDependencies={dependenciesFiltered}
