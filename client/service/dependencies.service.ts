...
import { getSettingsFromStorage } from '../app/settings.storage';
import { xCacheId } from '../xcache';
import { fetchJSON, getApiPath, getBasePathFor } from './utils';
...
export const getDependenciesScore = async (
  ...
  return dependenciesToQuery && dependenciesToQuery.length > 0
    ? await fetchJSON<BundleScore[]>(
        getApiPath(`/api/score/${dependenciesToQuery.join(',')}`),
      )
    : [];
};
...
export const getDependenciesDetails = async (
  ...
  return manager && dependenciesToQuery && dependenciesToQuery.length > 0
    ? await fetchJSON<BundleDetails[]>(
        getApiPath(`/api/details/${manager}/${dependenciesToQuery.join(',')}`),
      )
    : [];
};
...
export const installGlobalDependencies = async (
  ...
  const { useCurrentNpmForGlobalMutations } = getSettingsFromStorage();
  ...
  return fetchJSON(`${getBasePathFor('global')}`, {
    ...
    headers: {
      'x-cache-id': xCacheId,
      'x-global-npm-strategy': useCurrentNpmForGlobalMutations
        ? 'path'
        : 'owner',
    },
  });
