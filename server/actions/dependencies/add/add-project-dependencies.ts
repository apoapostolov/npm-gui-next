...
import { parseBunOutdated, parseBunPmList } from '../../bun-utils';
...
const getBunPackageWithInfo = async (
  projectPath: string,
  dependencyName: string,
): Promise<DependencyInstalled> => {
  const installedInfo = parseBunPmList(
    await executeCommandSimple(projectPath, 'bun pm ls'),
  );
  const outdatedInfo = parseBunOutdated(
    await executeCommandSimple(projectPath, 'bun outdated'),
  );
...
const addBunDependencies = async (
  projectPath: string,
  dependencies: Basic[],
  type: Type,
): Promise<DependencyInstalled | undefined> => {
  ...
  const command = `bun add ${dependenciesToInstall.join(' ')}${
    type === 'prod' ? '' : ' -d'
  }`;
  await executeCommandSimple(projectPath, command);

  if (dependencies.length === 1 && dependencies[0]) {
    return getBunPackageWithInfo(projectPath, dependencies[0].name);
  }
...
> = async ({
  ...
  if (manager === 'bun') {
    singleAddUpdate = await addBunDependencies(projectPathDecoded, body, type);
