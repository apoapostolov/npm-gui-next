...
describe(`Explorer`, () => {
  ...
  test('should mark bun lockfiles as project markers', async () => {
    const project = await prepareTestProject('explorer-bun', 'bun');
    await project.prepareClear({});

    const response = await api(app.server).get(
      `/api/explorer/${encodePath(
        path.join(__dirname, 'test-project-bun', 'explorer-bun'),
      )}`,
    );
