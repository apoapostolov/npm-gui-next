...
export const prepareTestProject = async (
  ...
  return {
    prepareClear: async ({
      ...
      await remove(path.join(testDirectoryPath, 'bun.lock'));
      await remove(path.join(testDirectoryPath, 'bun.lockb'));
      ...
      if (manager === 'bun') {
        await writeFile(path.join(testDirectoryPath, 'bun.lock'), '');
      }
      if (manager === 'yarn') {
...
const hasManagerInstalled = (manager: Manager): boolean =>
  spawnSync(manager, ['--version'], {
    shell: process.platform === 'win32',
  }).status === 0;

export const managers: Manager[] = ['npm', 'pnpm', 'yarn', 'bun'].filter(
  (manager): manager is Manager => hasManagerInstalled(manager as Manager),
);
...
export const BUN = {
  PKG_A: { ...NPM.PKG_A, manager: 'bun' },
  PKG_B: { ...NPM.PKG_B, manager: 'bun' },
  PKG_A_UNINSTALLED: { ...NPM.PKG_A_UNINSTALLED, manager: 'bun' },
  PKG_A_INSTALLED: { ...NPM.PKG_A_INSTALLED, manager: 'bun' },
  PKG_A_UP: { ...NPM.PKG_A_UP, manager: 'bun' },
  PKG_B_UP: { ...NPM.PKG_B_UP, manager: 'bun' },
  PKG_A_UP_INSTALLED: { ...NPM.PKG_A_UP_INSTALLED, manager: 'bun' },
  PKG_B_UP_INSTALLED: { ...NPM.PKG_B_UP_INSTALLED, manager: 'bun' },
  PKG_A_UP_NEWEST: { ...NPM.PKG_A_UP_NEWEST, manager: 'bun' },
  PKG_B_UP_NEWEST: { ...NPM.PKG_B_UP_NEWEST, manager: 'bun' },
};
